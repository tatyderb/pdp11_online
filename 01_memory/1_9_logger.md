...
```cpp
log_level = INFO;
trace(ERROR, "Для карты 123456789 введен неверный пин-код");
trace(INFO, "В систему зашел пользователь с именем Alex");
trace(DEBUG, "Это отладочная печать");
trace(WARNING, "Пользователь Alex пытается снять 1000 рублей, на счету 347 рублей");
```
напечатает:
```cpp
...
* `trace(level, message)` - функция логирования, в которой указан уровень логирования `level` и сообщение, которое нужно вывести, если этот уровень больше и равен установленному пороговому уровню логирования `log_level`.
...
trace(ERROR, "Ошибка 1");             // Ошибка 1
trace(INFO, "Информация 2");          // не напечатает ничего
old_log = set_log_level(DEBUG);       // old_log = WARNING
trace(INFO, "Информация 3");          // Информация 3
trace(DEBUG, "Отладка 4");            // Отладка 4
set_log_level(old_log);               // восстановили прежний пороговый уровень
trace(DEBUG, "Отладка 5");            // не напечатает ничего
trace(WARNING, "Предупреждение 6");   // Предупреждение 6
```
...
```cpp
trace(INFO, "Какой-то текст");
info("Какой-то текст");
```
Делать или нет функции-обертки `error`, `warning`, `info`, `trace_run`, `debug` - на ваше усмотрение.
...
* Реализуйте функцию `trace(level, format, ...)` с [переменным числом аргументов](https://stepik.org/lesson/311877/step/1)
...
int main()
{
    ...
    trace(INFO, "%d + %d = %d\n", x, y, x + y - 1);
    trace(ERROR, "Oops, %d+%d=%d, not %d\n", x, y, x + y, x + y - 1);
    trace(TRACE, "Эту надпись не должны видеть\n");
    
    int old_log_level = set_log_level(TRACE);
    
    trace(TRACE, "Visible text\n");
    trace(DEBUG, "Debug info\n");
    
    set_log_level(old_log_level);
    
    trace(INFO, "The end!\n");
    trace(TRACE, "No code after return\n");

...
Пусть где-то существует компьютер. У него только 4 ячейки памяти. Каждая ячейка размером в 8 бит. Эти ячейки имеют название и **адрес** по которому обращаются к этому регистру.
